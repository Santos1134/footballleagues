-- VERIFY AND FIX CUP ADMIN ACCESS
...
-- Step 2: Verify cup admin RLS policies exist
SELECT
  tablename,
  policyname,
  cmd
...
AND tablename IN ('cups', 'cup_groups', 'cup_teams', 'cup_matches', 'cup_players')
ORDER BY tablename, policyname;

-- Step 3: Fix cups table policies
...
-- Step 4: Fix cup_groups policies
...
-- Step 6: Fix cup_matches policies
...
-- Step 7: Fix cup_players policies
...
-- Step 9: Check that cup admins can only see their cup
-- (Run this while logged in as a cup admin to test)
SELECT
  id,
  name,
  'You should only see the cup you manage' as note
FROM cups;
