...
export default async function AdminDashboard() {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('role, managed_league_id, managed_cup_id')
    .eq('id', user.id)
    .single()
  ...
  const isSuperAdmin = profile.role === 'admin' && !profile.managed_league_id && !profile.managed_cup_id
  const isLeagueAdmin = profile.role === 'league_admin' && profile.managed_league_id !== null
  const isCupAdmin = profile.role === 'league_admin' && profile.managed_cup_id !== null
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Leagues Section - Only for Super Admin and League Admin */}
          {(isSuperAdmin || isLeagueAdmin) && (
            <>
          ...
          {/* System Section - Available to all admin types */}
          <div className="bg-white rounded-lg shadow-lg overflow-hidden border-t-4 border-liberia-red">
            <div className="bg-gradient-to-r from-liberia-red to-liberia-red-dark text-white px-6 py-4">
